...
var Map = (function (KeyedCollection$$1) {
  ...
  Map.prototype.diffFrom = function diffFrom (otherMap) {
    ...
    var update$$1 = function (prevNext, key) { return updated.set(key, prevNext); };

    this.diffFromCallbacks(otherMap, { add: add, remove: remove, update: update$$1 });
...
ArrayMapNode.prototype.iterateOverEntries = function iterateOverEntries (callback) {
...
BitmapIndexedNode.prototype.getHashRanges = function getHashRanges () {
    var this$1 = this;
  ...
  while (bitmap !== 0) {
    if (bitmap & (1 === 1)) {
      var subNode = this$1.nodes[position];
      hashRanges.push({ hash: hash$$1, node: subNode });
      position += 1;
    }
    bitmap >>>= 1;
    hash$$1++;
...
BitmapIndexedNode.prototype.iterateOverEntries = function iterateOverEntries (callback) {
  this.nodes.forEach(function (node) { return !!node && node.iterateOverEntries(callback); });
};
...
HashArrayMapNode.prototype.getHashRanges = function getHashRanges () {
  return this.nodes
...
HashArrayMapNode.prototype.iterateOverEntries = function iterateOverEntries (callback) {
  this.nodes.forEach(function (node) { return !!node && node.iterateOverEntries(callback); });
};
...
HashCollisionNode.prototype.iterateOverEntries = function iterateOverEntries (callback) {
  this.entries.forEach(callback);
};
...
ValueNode.prototype.iterateOverEntries = function iterateOverEntries (callback) {
  callback(this.entry);
};
...
function processAllEntries(node1, node2, add, remove, update$$1) {
  if (!node1) {
    node2.iterateOverEntries(function (ref) {
      var key = ref[0];
      var value = ref[1];
  ...
  if (!node2) {
    node1.iterateOverEntries(function (ref) {
      var key = ref[0];
      var value = ref[1];
  ...
  var node1Entries = [];
  node1.iterateOverEntries(function (entry) {
  ...
  node2.iterateOverEntries(function (ref) {
    ...
    var oldEntryIndex = node1Entries.findIndex(
      function (oldEntry) { return oldEntry !== null && oldEntry[0] === key; }
    );
    if (oldEntryIndex === -1) {
      add(value, key);
    } else {
      var oldEntry = node1Entries[oldEntryIndex];
      if (oldEntry[1] !== value) {
        update$$1({ prev: oldEntry[1], next: value }, key);
      }
      node1Entries[oldEntryIndex] = null;
    }
  });
  node1Entries.forEach(function (entry) {
    if (entry !== null) {
...
function processDiffForEquivalentNodes(node1, node2, add, remove, update$$1) {
  if (node1 === node2) {
  ...
  var hashRanges1 =
    node1 && node1.getHashRanges ? node1.getHashRanges() : undefined;
  var hashRanges2 =
    node2 && node2.getHashRanges ? node2.getHashRanges() : undefined;

  if (!hashRanges1 || !hashRanges2) {
    return processAllEntries(node1, node2, add, remove, update$$1);
  }
  ...
  while (hashIndex1 < hashRanges1.length && hashIndex2 < hashRanges2.length) {
    var ref = hashRanges1[hashIndex1];
    var subNode1 = ref.node;
    var hash1 = ref.hash;
    var ref$1 = hashRanges2[hashIndex2];
    var subNode2 = ref$1.node;
    var hash2 = ref$1.hash;
    if (hash1 < hash2) {
      processAllEntries(subNode1, undefined, add, remove, update$$1);
      hashIndex1 += 1;
    } else if (hash2 < hash1) {
      processAllEntries(undefined, subNode2, add, remove, update$$1);
      hashIndex2 += 1;
    } else {
      processDiffForEquivalentNodes(subNode1, subNode2, add, remove, update$$1);
      hashIndex1 += 1;
      hashIndex2 += 1;
    }
  }
  while (hashIndex1 < hashRanges1.length) {
    var ref$2 = hashRanges1[hashIndex1];
    var subNode1$1 = ref$2.node;
    processAllEntries(subNode1$1, undefined, add, remove, update$$1);
    hashIndex1 += 1;
  }
  while (hashIndex2 < hashRanges2.length) {
    var ref$3 = hashRanges2[hashIndex2];
    var subNode2$1 = ref$3.node;
    processAllEntries(undefined, subNode2$1, add, remove, update$$1);
    hashIndex2 += 1;
  }
}
...
var Set = (function (SetCollection$$1) {
  ...
  Set.prototype.diffFrom = function diffFrom (otherSet) {
    var ref = this._map.diffFrom(otherSet._map);
    var added = ref.added;
